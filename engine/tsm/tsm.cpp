...
u_int64_t TSM::write_data_to_file<string>(
        const DataBlock<string> & data_block ,
        const string & file_path,
        int64_t offset) const
{
    auto & file = FileManager::get_output_stream(file_path);
        ...
        int64_t offset) const
{
    ...
    file.write(reinterpret_cast<const char*>(&index_entry.get_offset()), sizeof(int64_t));
...
bool TSM::write_index_meta_to_file(
        const IndexBlockMeta & index_meta,
        const string & file_path,
        int64_t offset) const
{
