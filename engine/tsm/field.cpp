        ...
        const std::string & series_key_and_field_name)
{
    ...
    if (sl->m_need_reset.load())
    {
        ...
        m_sl.notify(db_name, tb_name, shard_id, m_field_name, true, false);  // 注册事件
    ...
    if (should_flush_data(sl->m_sl_last_time, sl->size()))
    {
        ...
        sl->cle();
        // 刷了块取消监控事件
        m_sl.notify(db_name, tb_name, shard_id, m_field_name, false, false);
        std::cout << "数据已入队列...\n";
        // 使下一次写入数据时初始化时间戳
...
//        m_sl.notify(db_name, tb_name, shard_id, m_field_name, true, false);
//        std::cout << "注册事件, table: " << tb_name << ", field:" << m_field_name << std::endl;
//    }
//
//    /**
...
//        m_current_data->m_min_timestamp = m_sl.min_key();
